...
self.addEventListener('install', (evetn) => {
  console.log("[ServiceWorker] The service-worker was installed");
})
...
self.addEventListener('fetch', (event) => {
  console.log("[ServiceWorker] Detected a fetch event");
  event.respondWith(
    caches.open(CURRENT_CACHES['read-through']).then((cache) => {
      console.log("[ServiceWorker]");
      console.log(event.request);
      return cache.match(event.request).then((response) => {
        if (response) { // If there is an entry in the cache for event.request, then response will be defined and we can just return it.
          return response;
        }

        // Otherwise, if there is no entry in the cache for event.request, response will be
        // undefined, and we need to fetch() the resource.
        console.log('[ServiceWorker] No response for %s found in cache. About to fetch from network...', event.request.url);
